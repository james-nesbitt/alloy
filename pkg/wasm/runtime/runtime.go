...
func (r *Runtime) witLog(ctx context.Context, mod wazeroapi.Module, levelPtr, levelLen, msgPtr, msgLen uint32) {
	...
	msg := string(msgData)
	switch level {
	case "debug": r.logger.Debug("wasm_log", "plugin", mod.Name(), "msg", msg)
	case "info":  r.logger.Info("wasm_log", "plugin", mod.Name(), "msg", msg)
	case "warn":  r.logger.Warn("wasm_log", "plugin", mod.Name(), "msg", msg)
	case "error": r.logger.Error("wasm_log", "plugin", mod.Name(), "msg", msg)
	default:      r.logger.Info("wasm_log", "plugin", mod.Name(), "level", level, "msg", msg)
	}
}
...
func (r *Runtime) witRouteMessage(ctx context.Context, mod wazeroapi.Module, msgPtr, msgLen uint32) {
	...
	if err := json.Unmarshal(data, &wmsg); err == nil {
		...
		if wmsg.Target.Set { apiMsg.Target = wmsg.Target.Value }
...
func (r *Runtime) witCall(ctx context.Context, mod wazeroapi.Module, msgPtr, msgLen, respPtrPtr, respSizePtr uint32) uint32 {
	...
	if err := json.Unmarshal(data, &wmsg); err == nil {
		...
		if resp.Target != "" { wresp.Target = witOption[string]{Value: resp.Target, Set: true} }
...
func (r *Runtime) witGetNextMessage(ctx context.Context, mod wazeroapi.Module, ptrPtr, sizePtr uint32) uint32 {
	...
	case msg := <-instance.msgChan:
		...
		if msg.Target != "" { wmsg.Target = witOption[string]{Value: msg.Target, Set: true} }
...
func (r *Runtime) RouteMessage(ctx context.Context, pluginID string, msg api.Message) error {
	...
	select {
	case instance.msgChan <- msg: return nil
	case <-ctx.Done(): return ctx.Err()
	default: return errors.New("channel full")
	}
}
...
func (r *Runtime) GetResponse(ctx context.Context, pluginID string) (api.Message, error) {
	...
	instance, ok := r.plugins[pluginID]
	r.mu.RUnlock()
	if !ok { return api.Message{}, errors.New("plugin not found") }
	select {
	case resp := <-instance.respChan: return resp, nil
	case <-ctx.Done(): return api.Message{}, ctx.Err()
	case <-time.After(1 * time.Second): return api.Message{}, errors.New("timeout")
	}
}
...
func (l *loggerWriter) Write(p []byte) (n int, err error) {
	...
	for {
		idx := -1
		for i, b := range l.buf { if b == '\n' { idx = i; break } }
		if idx == -1 { break }
		line := string(l.buf[:idx])
		l.buf = l.buf[idx+1:]
		if l.stream == "stdout" { l.logger.Info("wasm_out", "id", l.id, "msg", line) }
		if l.stream == "stderr" { l.logger.Error("wasm_err", "id", l.id, "msg", line) }
	}
	return len(p), nil
}
